...
export default function Movement() {
  ...
  const { data: entries = [], isLoading } = useRecentMovementEntries(TREND_DAYS);

  const stats = useMemo(() => computeStats(days), [days]);
  const dailyValues = days.map((d) => d.burned);
  const max = Math.max(1, ...dailyValues);
  const activeDays = days.filter((d) => d.burned > 0).length;
    ...
    <SafeAreaView className="flex-1 bg-cream" edges={['top', 'bottom']}>
      ...
      <ScrollView
        ...
        <View className="px-8 pt-8">
          <Text className="text-[15px] text-graphite">You burned today</Text>
          <View className="flex-row items-baseline mt-1">
            <Text
              className="font-serif-light text-sage"
          ...
          {stats.avg7 !== null && stats.today > 0 && (
            <Text className={`text-[13px] mt-2 ${vsAvgTint(stats.today - stats.avg7)}`}>
        ...
        <View className="px-8 pt-12">
          ...
          <Bars values={dailyValues} max={max} tint="sage" height={80} />
          ...
          {isLoading ? (
            ...
            entries.map((e, i) => (
              <EntryRow
                ...
                accent="sage"
                onPress={() =>
                  router.push({ pathname: '/detail', params: { type: 'movement', id: e.id } })
                }
              />
            ))
          )}
...
function computeStats(days: { date: string; burned: number }[]): Stats {
  if (days.length === 0) return { today: 0, avg7: null, avg30: null };
  const todayKey = localDateKey(new Date());
  const todayBucket = days.find((d) => d.date === todayKey);
  const today = todayBucket?.burned ?? 0;

  const last7 = days.slice(-7).filter((d) => d.burned > 0);
  const last30 = days.filter((d) => d.burned > 0);

  const avg = (arr: { burned: number }[]) =>
    arr.length === 0 ? null : Math.round(arr.reduce((s, d) => s + d.burned, 0) / arr.length);
...
function vsAvgTint(delta: number): string {
  if (Math.abs(delta) < 5) return 'text-graphite';
  return delta > 0 ? 'text-sage' : 'text-terracotta';
}
