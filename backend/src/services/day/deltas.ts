 ...
 */
export function deltaVsLast(current: DayActivity, previous: DayActivity | null): DeltaVsLast {
	if (!previous) {
		return {
			text: "first time",
			direction: "new",
	...
	if (!moved) {
		return { text: "same", direction: "same", field: null, load_lb: load, sets, reps, previous: was };
	}
...
export function withDeltas(
	...
	return chronological.map((activity) => {
		...
		seenToday.push(activity);
		return { activity, delta_vs_last: deltaVsLast(activity, previous) };
	});
}
