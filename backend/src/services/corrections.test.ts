...
const weight = (over: Partial<Extract<FusionResult, { kind: "weight" }>> = {}): FusionResult => ({
	kind: "weight",
	weight_lb: 182.4,
	confidence: "high",
	sources: null,
...
describe("diffFields", () => {
	it("reports only what moved, in the order the fields are named", () => {
		const changes = diffFields(
			{ exercise: "Row", equipment: "machine", description: "row", category: "strength", sets: 3, reps: 12, load_lb: 45 },
			{ exercise: "Row", equipment: "machine", description: "row", category: "strength", sets: 3, reps: 12, load_lb: 50 },
			CORRECTABLE_FIELDS.activity
		);
		expect(changes).toEqual([{ field: "load_lb", from: 45, to: 50 }]);
	});
	...
	it("ignores a field the patch never named", () => {
		...
		expect(diffFields({ kcal: 900, description: "row" }, { kcal: 900 }, CORRECTABLE_FIELDS.activity)).toEqual([]);
	});
});

describe("diffResults", () => {
	it("measures the field that changed, and nothing else", () => {
		const corrections = diffResults([weight()], [weight({ weight_lb: 183.2 })], "reweighed this morning");
		expect(corrections).toEqual([
			{
				part: 0,
				item: null,
				instruction: "reweighed this morning",
				changes: [{ field: "weight_lb", from: 182.4, to: 183.2 }],
			},
		]);
	});

	it("says nothing about a part the instruction did not touch", () => {
		// Every part is re-read on a revision, so "reweighed this morning" reaches the run too.
		// A part that came back as it went in is not history.
		const before = [weight(), activity()];
		const after = [weight({ weight_lb: 183.2 }), activity()];
		const corrections = diffResults(before, after, "reweighed this morning");
	...
	it("refuses to invent a diff when the shape changed under it", () => {
		// A revision that dropped an exercise, or turned a weigh-in into something else, is
		// not a field-level correction and a fictional diff is worse than none.
		expect(diffResults([activity()], [{ kind: "activities", items: [] } as unknown as FusionResult], "drop it")).toEqual([]);
		expect(diffResults([weight()], [activity()], "that was a workout")).toEqual([]);
	});
	...
	it("has nothing to say about a goal, a statement or a question", () => {
		const goal: FusionResult = {
			kind: "goal",
			spec: { kind: "custom", title: "Down to 170", metrics: [], active_from: null, active_to: null },
